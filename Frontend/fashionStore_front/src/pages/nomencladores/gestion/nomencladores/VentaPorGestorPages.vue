<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Ventas_Por_Gestor" />
        </q-breadcrumbs>
            ...
            <template v-slot:top>
                <div class="col-4 q-table__title">
                    ...
                    <q-select
                        ...
                        map-options
                        :use-input="
                            gestorSeleccionado === null || gestorSeleccionado === ''
                        "
                        option-label="nombre"
                        option-value="id"
                        :options="filtradoGestores"
                        @filter="
                            (val, update) => {
                                filtradoGestores = filterOptions(
                                val,
                                update,
                                filtradoGestores,
                                'nombre',
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnVentaPorGestor } from "src/assets/js/column_data/columnDataGestion";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnVentaPorGestor;
const filter = ref("");
const items = ref([]);
 ...
 *                                         LISTAS                                                  *
 **************************************************************************************************/
const itemsGestores = ref([])
const filtradoGestores = ref([])
...
onMounted(async () => {
    ...
    itemsGestores.value = (await loadGet('Gestor/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
    console.log("itemsGestores: ",itemsGestores)
...
const obtenerElementoPorId = async (id) => {
    filtradoGestores.value = itemsGestores.value
