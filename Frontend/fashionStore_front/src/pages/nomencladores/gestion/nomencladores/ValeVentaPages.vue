<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="ValeVenta" />
        </q-breadcrumbs>
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnValeVenta } from "src/assets/js/column_data/columnDataGestion";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnValeVenta;
const filter = ref("");
const items = ref([]);
...
onMounted(async () => {
    ...
    items.value = (await loadGet('ValeVenta/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const Guardar = () => {
    const url = objeto.id ? 'ValeVenta/Actualizar' : 'ValeVenta/Crear'
    saveData(url, objeto, load, close, dialogLoad)
}

// Funcion para Obtener los datos para editar
const obtenerElementoPorId = async (id) => {
    await obtener('ValeVenta/ObtenerPorId', id, objeto, dialogLoad, dialog)
}
...
const load = async () => {
    items.value = await loadGet('ValeVenta/ObtenerListadoPaginado')
}
