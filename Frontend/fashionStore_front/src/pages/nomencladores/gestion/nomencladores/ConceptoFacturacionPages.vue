<template>
    <div class="q-pa-xl">
        <q-breadcrumbs class="qb cursor-pointer q-pb-md">
            ...
            <q-breadcrumbs-el label="Nomencladores" icon="dashboard" @click="router.push('/facturacion')" />
            <q-breadcrumbs-el label="ConceptoFacturacion" />
        </q-breadcrumbs>
            ...
            <header class="q-pa-sm bg-primary">
                <q-toolbar>
                    <q-toolbar-title class="text-subtitle6 text-white">
                        {{
                            objeto?.id ? "Editar Concepto de Facturación" : "Adicionar Concepto de Facturación"
                        }}</q-toolbar-title>
                </q-toolbar>
            </header>
            <q-form @submit.prevent="Guardar()" @reset="close" ref="myForm">
                <div class="h row q-ma-md">
                    <q-input class="col-xs-12 col-sm-5" label="Código*" v-model="objeto.codigoConceptoFacturacion" color="primary" :disable="!!objeto.id"
                        counter maxlength="255" :rules="[onlyLetter_Number_No_White_Spaces, string, (val) =>
                            (items.length > 0
                                ? !isValorRepetido(val, 'codigoConceptoFacturacion', objeto, items)
                    ...
                    <q-select
                        ...
                        v-model="objeto.conceptoProduccionId"
                        label="Concepto de Producción *"
                        emit-value
                        map-options
                        :use-input="
                            objeto.conceptoProduccionId === null || objeto.conceptoProduccionId === ''
                        "
                        option-label="descripcion"
                        option-value="id"
                        :options="filtradoConceptoProduccion"
                        @filter="
                            (val, update) => {
                                filtradoConceptoProduccion = filterOptions(
                                val,
                                update,
                                filtradoConceptoProduccion,
                                'descripcion',
                                itemsConceptoProduccion
                            );
                            }
                        "
                        lazy-rules
                    ...
                    <q-input class="col-xs-12 " label="Descripción*" v-model="objeto.descripcion" color="primary"
                        counter maxlength="255" :rules="[onlyLetter_Number, string, (val) =>
                            (items.length > 0
                                ? !isValorRepetido(val, 'descripcion', objeto, items)
                                : true) || 'Ya existe una descripción con ese valor',]" />
                    <q-checkbox class="q-mr-md q-mt-sm" right-label v-model="objeto.cargo" :disable="!!objeto.id"
                        label="Cargo" color="primary" />
                    <q-checkbox v-if="objeto.id && !objeto?.id" class="q-mt-sm" right-label v-model="objeto.activo"
...
<script setup>
// IMPORT LIBRERIES
import { dataColumnConcepto_Facturacion } from "src/assets/js/column_data/columnDataFacturacion";
import {
    loadGet,
    saveData,
    isValorRepetido,
...
// DATOS DE LA TABLA
const columns = dataColumnConcepto_Facturacion;
const filter = ref("");
const items = ref([]);
 ...
 **************************************************************************************************/
const objetoInicial = {
    codigoConceptoFacturacion: null,
    descripcion: null,
    conceptoProduccionId: null,
    cargo: true,
    activo: true
}
 ...
 *                                         LISTAS                                                  *
 **************************************************************************************************/
const itemsConceptoProduccion = ref([])
const filtradoConceptoProduccion = ref([])
...
onMounted(async () => {
    ...
    items.value = (await loadGet('ConceptoFacturacion/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
    itemsConceptoProduccion.value = (await loadGet('ConceptoProduccion/ObtenerListadoPaginado')) ?? []; // Condicion para en caso de error la tabla no de error ya q la api devulve undefined
...
const Guardar = () => {
    const url = objeto.id ? 'ConceptoFacturacion/Actualizar' : 'ConceptoFacturacion/Crear'
    saveData(url, objeto, load, close, dialogLoad)
}

// Funcion para Obtener los datos para editar
const obtenerElementoPorId = async (id) => {
    filtradoConceptoProduccion.value = itemsConceptoProduccion.value
    await obtener('ConceptoFacturacion/ObtenerPorId', id, objeto, dialogLoad, dialog)
...
const load = async () => {
    items.value = await loadGet('ConceptoFacturacion/ObtenerListadoPaginado')
}
